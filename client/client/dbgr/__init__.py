...
try:
    ...
    try:
        ...
        try:
            __version__ = pkg_resources.require('dbgr')[0].version
        except pkg_resources.DistributionNotFound:
            __version__ = "dbgr is not installed"
else:
    try:
        __version__ = _pkg_version('dbgr')
...
# Get dbgr server host
SOCKET_SERVER = os.getenv('DBGR_SOCKET_SERVER', 'localhost')
# and port
SOCKET_PORT = int(os.getenv('DBGR_SOCKET_PORT', '19840'))

# Get dbgr web server host
WEB_SERVER = os.getenv('DBGR_WEB_SERVER')
# and port
WEB_PORT = int(os.getenv('DBGR_WEB_PORT', 0))

DBGR_NO_BROWSER_AUTO_OPEN = bool(os.getenv('DBGR_NO_BROWSER_AUTO_OPEN', False))
log = logger('dbgr')
trace_log = logging.getLogger('dbgr.trace')

for log_name in ('main', 'trace', 'ui', 'ext', 'bp'):
    logger_name = 'dbgr.%s' % log_name if log_name != 'main' else 'dbgr'
    level = os.getenv(
        'DBGR_%s_LOG' % log_name.upper(), os.getenv('DBGR_LOG', 'WARNING')
    ).upper()
    logging.getLogger(logger_name).setLevel(getattr(logging, level, 'WARNING'))


class Dbgr(object):
    """Dbgr debugger main class"""
    ...
    def get(no_create=False, server=None, port=None, force_uuid=None):
        ...
        dbgr = Dbgr._instances.get((pid, thread))
        if not dbgr and not no_create:
            dbgr = object.__new__(Dbgr)
            Dbgr.__init__(dbgr, server, port, force_uuid)
            dbgr.pid = pid
            dbgr.thread = thread
            Dbgr._instances[(pid, thread)] = dbgr
        elif dbgr:
            if (
                server is not None
                and dbgr.server != server
                or port is not None
                and dbgr.port != port
            ):
                log.warn('Different server/port set, ignoring')
            else:
                dbgr.reconnect_if_needed()
        return dbgr
    ...
    def pop():
        ...
        pid = os.getpid()
        thread = threading.current_thread()
        Dbgr._instances.pop((pid, thread))
    ...
    def __init__(self, server=None, port=None, force_uuid=None):
        log.debug('New dbgr instance %r' % self)
        self.obj_cache = {}
        self.compile_cache = {}
        self.tracing = False
        self.begun = False
    ...
    def run(self, cmd, fn=None, globals=None, locals=None):
        ...
        if isinstance(cmd, str):
            str_cmd = cmd
            cmd = compile(str_cmd, fn or "<dbgr>", "exec")
    ...
    def connect(self):
        ...
        while not self._socket and tries < 10:
            try:
                ...
                tries += 1
                log.warning(
                    'You must start/install dbgr.server '
        ...
        Dbgr._sockets.append(self._socket)
    ...
    def index_imports(self):
        ...
        index_thread = Thread(
            target=index, args=(self,), name='dbgr_importmagic_build_index'
        )
    ...
    def better_repr(self, obj, context=None, html=True, level=1, full=False):
        ...
        if isinstance(obj, dict):
            if isinstance(obj, OrderedDict):
                ...
                def dict_sorted(it, key):
                    try:
                        return sorted(it, key=key)
                    except TypeError:
                        try:
                            return sorted(it, key=lambda x: str(x[0]))
                        except Exception:
                            return list(it)
    ...
    def open_browser(self, type_='debug'):
        if not self.connected:
            ...
            server = WEB_SERVER or '[dbgr.server]'
            if WEB_PORT:
                server += ':%s' % WEB_PORT

            if DBGR_NO_BROWSER_AUTO_OPEN:
                log.warning(
...
def set_trace(frame=None, skip=0, server=None, port=None):
    ...
    for i in range(skip):
        if not frame.f_back:
            break
        frame = frame.f_back
    dbgr = Dbgr.get(server=server, port=port)
    dbgr.set_trace(frame)
    return dbgr
...
def start_trace(
    ...
    dbgr = Dbgr.get(server=server, port=port)
    if not dbgr.stepping:
        dbgr.start_trace(full, frame or sys._getframe().f_back, below, under)
    return dbgr
...
def stop_trace(frame=None, close_on_exit=False):
    """Stop tracing"""
    log.info('Stopping trace')
    dbgr = Dbgr.get(True)  # Do not create an istance if there's None
    if dbgr and (not dbgr.stepping or close_on_exit):
        log.info('Stopping trace')
        dbgr.stop_trace(frame or sys._getframe().f_back)
        if close_on_exit:
            dbgr.die()
    return dbgr
...
def shell(source=None, vars=None, server=None, port=None):
    ...
    Dbgr.get(server=server, port=port).shell(source=source, vars=vars)
...
def post_mortem(t=None, server=None, port=None):
    ...
    dbgr = Dbgr.get(server=server, port=port)
    dbgr.reset()
    dbgr.interaction(None, t, post_mortem=True)
