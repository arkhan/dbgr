...
# Max children returned per Expand request (avoid huge payloads on big containers)
MAX_CHILDREN = 500
# Max chars of the collapsed one-line preview for expandable variables
MAX_PREVIEW = 200


def is_expandable(val):
    """True if val has children worth showing in the variables tree"""
    if isinstance(val, (dict, list, tuple, set, frozenset)):
        return len(val) > 0
    if isinstance(val, (str, bytes, int, float, bool, complex, type(None))):
        return False
    if callable(val):
        return False
    if getattr(val, '__dict__', None):
        return True
    if getattr(type(val), '__slots__', None):
        return True
    return False
...
class Interaction(object):
        ...
        while not stop:
            ...
            try:
                ...
                try:
                    ...
                    link = (
                        '<a href="https://github.com/arkhan/dbgr/issues/new?'
    ...
    def _var_entry(self, val):
        """Build a {val, type, expandable, len} entry for the variables tree.

        Expandable containers/objects get a cheap plain-repr preview (children
        are fetched lazily via Expand); everything else gets the existing
        rich safe_better_repr (may include an inspect link)."""
        expandable = is_expandable(val)
        if expandable:
            preview = self.db.safe_repr(val)
            if len(preview) > MAX_PREVIEW:
                preview = preview[:MAX_PREVIEW] + u('…')
            entry = {'val': preview, 'expandable': True}
        else:
            entry = {
                'val': self.db.safe_better_repr(val),
                'expandable': False,
            }
        ...
        try:
            ...
            try:
                entry['len'] = len(vars(val))
            except Exception:
                entry['len'] = None
        return entry

    def _error_entry(self, e):
        return {
            'val': '%s: %s' % (type(e).__name__, e),
            'type': 'error',
            'expandable': False,
            'len': None,
        }
    ...
    def update_watchers(self):
        ...
        # Auto-include all locals (pudb-style), skipping dunder noise
        # (__name__, __file__, __builtins__…) that clutters module-level
        # frames without being an actual variable of interest.
        try:
            for name, val in sorted(self.current_locals.items()):
                if name.startswith('__') and name.endswith('__'):
                    continue
                # Skip function/class/module definitions and imports: at
                # module scope locals==globals, so these would otherwise
                # flood the panel with code, not state (pudb doesn't hit
                # this because it always shows f_locals of a *function*
                # frame, which normally has none of these).
                if inspect.isroutine(val) or inspect.isclass(val) or inspect.ismodule(val):
                    continue
                try:
                    watched[name] = self._var_entry(val)
                except Exception as e:
                    watched[name] = self._error_entry(e)
        except Exception:
            pass

        # Explicit user-added watchers (expressions)
        for watcher in self.db.watchers[self.current_file]:
            try:
                val = eval_(watcher, self.get_globals(), self.current_locals)
                entry = self._var_entry(val)
            except Exception as e:
                entry = self._error_entry(e)
            entry['watch'] = True
            watched[watcher] = entry
    ...
    def do_expand(self, data):
        """Lazy-load children of an expandable variable for the tree view"""
        payload = loads(data)
        path = payload['path']
        try:
            obj = eval_(path, self.get_globals(), self.current_locals)
        except Exception as e:
            self.db.send(
                'Expanded|%s'
                % dump(
        ...
        try:
            if isinstance(obj, dict):
                for key, val in list(obj.items())[:MAX_CHILDREN]:
                    key_repr = self.db.safe_repr(key)
                    entry = self._var_entry(val)
                    entry['path'] = '(%s)[%s]' % (path, key_repr)
                    children[key_repr] = entry
            elif isinstance(obj, (list, tuple)):
                for i, val in enumerate(obj[:MAX_CHILDREN]):
                    entry = self._var_entry(val)
                    entry['path'] = '(%s)[%d]' % (path, i)
                    children[str(i)] = entry
            elif isinstance(obj, (set, frozenset)):
                for i, val in enumerate(list(obj)[:MAX_CHILDREN]):
                    entry = self._var_entry(val)
                    entry['path'] = 'list(%s)[%d]' % (path, i)
                    children[str(i)] = entry
            else:
                ...
                for name, val in sorted(attrs.items())[:MAX_CHILDREN]:
                    entry = self._var_entry(val)
                    entry['path'] = '(%s).%s' % (path, name)
                    children[name] = entry
        except Exception:
            log.exception('Error expanding %s' % path)

        self.db.send(
            'Expanded|%s' % dump({'path': path, 'children': children})
        )

    def do_setvar(self, data):
        """Inline edit: assign a new value to a variable/attribute/item"""
        payload = loads(data)
        expr = payload['expr']
        value = payload['value']
        try:
            compiled_code = compile(
                '%s = (%s)' % (expr, value), '<stdin>', 'exec'
            )
            execute(compiled_code, self.get_globals(), self.current_locals)
        except Exception as e:
            self.db.send(
                'Echo|%s'
                % dump(
