...
log = get_color_logger('dbgr.test')
log.info('Conftest')
log.setLevel(getattr(logging, os.getenv('DBGR_TEST_LOG', 'WARNING')))
GLOBALS = globals()
LOCALS = locals()
...
class Slave(Process):
    def __init__(self, use, host='localhost', port=19999):
        ...
        if use.with_main:
            ...
            self.file = os.path.join(
                os.path.dirname(__file__), '..', 'client', 'dbgr', '__main__.py'
            )
    ...
    def run(self):
        ...
        dbgr.SOCKET_SERVER = self.host
        dbgr.SOCKET_PORT = self.port
...
def socket(request):
    ...
    if not os.getenv('NO_DBGR_TIMEOUT'):
        signal.alarm(5)
